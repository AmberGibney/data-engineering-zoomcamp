...
docker run -it \
  ...
  -e POSTGRES_PASSWORD="root" \
  -e POSTGRES_DB="ny_taxi" \
  -v /home/ambergibney/data-engineering-zoomcamp/week_1_basics_n_setup/2_docker_sql/data/ny_taxi_postgres_data:/var/lib/postgresql/data \
  -p 5432:5432 \
  --network=pg-network \
  --name pg-database \
  postgres:13
...
docker run -it \
  ...
  taxi_ingest:v001 \
    ...
    --port=5432 \
    --db=ny_taxi \
    --table_name=yellow_taxi_trips_new \
    --url=${URL}
```
